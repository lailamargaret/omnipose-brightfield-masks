...
def create_merged_directory(input_dir, base_dir):
    print(f'Making merged directory.')
...
def make_multi_channels(input_dir, subdir, output_dir, num_workers=4):
    if input_dir != output_dir:
        print(f"Making multi-channel merged images for {subdir}")
...
def make_multi_channels_reorder_old(input_dir, subdir, output_dir, num_workers=4):
    if input_dir != output_dir:
        print(f"Making multi channels {subdir}")
        subdir_path = os.path.join(input_dir, subdir)
        merged_subdir = os.path.join(output_dir, subdir)
...
def make_multi_channels_reorder_testreplace(input_dir, subdir, output_dir, num_workers=4):
    if input_dir == output_dir:
        return subdir
    ...
    z_stack_dict = {}
    for file in tif_files:
        match = re.match(r'([A-Z]\d+)_pos(\d+)Z(\d+)_', file)
        if match:
            well = match.group(1)
            pos = f"pos{match.group(2)}"
            z = int(match.group(3))
            key = f"{well}_{pos}"
            if key not in z_stack_dict:
                z_stack_dict[key] = {}
            z_stack_dict[key][z] = os.path.join(subdir_path, file)

    merged_pairs = {}
    for key, z_files in z_stack_dict.items():
        if all(z in z_files for z in [0, 1, 2, 3]):
            files = [z_files[2], z_files[3], z_files[1]]  # Z2, Z3, Z1
            output_name = f"{key}_merged_z2z3z1.tif"
            merged_pairs[output_name] = files
        elif all(z in z_files for z in [0, 1, 2]):
            files = [z_files[1], z_files[2], z_files[0]]  # Z1, Z2, Z0
            output_name = f"{key}_merged_z1z2z0.tif"
            merged_pairs[output_name] = files

    for output_name, file_paths in merged_pairs.items():
        if output_name not in existing_merged_files:
            process_images(file_paths, merged_subdir, output_name)
...
def make_multi_channels_reorder(input_dir, subdir, output_dir, num_workers=4):
    ...
    existing_merged_files = set(img for img in os.listdir(merged_subdir)
        if img.endswith('.tif') and not any(suffix in img for suffix in ['masks.tif', 'dP.tif', 'outlines.tif', 'flows.tif', 'Wells']))
    ...
    for file in tif_files:
        ...
        if match:
            ...
            z = int(match.group(3))
            key = f"{well}_{pos}"
            z_stack_dict.setdefault(key, {})[z] = os.path.join(subdir_path, file)
    ...
    def worker(args):
        output_name, file_paths = args
        #print(f"[{threading.current_thread().name}] Processing {output_name}")
        process_images(file_paths, merged_subdir, output_name)

    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        executor.map(worker, merged_pairs.items())

    return subdir

# if __name__ == "__main__":
#     directories = [(r"Z:\Wellslab\Cytation_overflow_040723\20250606_rainbowsnaps_d1\rainbowsnaps1_20250606_151556", r"C:\Users\laila\Documents\20250606_rainbowsnaps_d1\merged")]
#     print(f'Merging brightfield stacks')
#     # input_directory = r"Z:\Wellslab\Cytation_overflow_040723\20250508_10X_D-5a_DMSOday1\D-5a 20250508_20250508_111831"
#     # output_directory = r"C:\Users\laila\Documents\20250508_10X_D-5a\merged"
#     # For default multi-channel merge:
#     for (input_directory, output_directory) in directories:
#         make_multi_channels_reorder(input_directory, "", output_directory)
